...
const Dashboard: React.FC<DashboardProps> = ({ user, onOpenSupport, onOpenCheckout, onOpenGame, onRefresh, isRefreshing, showPalette, setShowPalette }) => {
  ...
  const [showPassword, setShowPassword] = useState(false); // Toggle password visibility in modal
  ...
  const containerClass = bgImage ? 'bg-black/50 min-h-screen pb-32 backdrop-blur-sm' : `${activeTheme.bgClass} min-h-screen pb-32 transition-colors duration-500 will-change-contents text-base`;
  ...
  const handleServiceClick = (rawService: string) => {
      ...
      // Find the assigned credential robustly using exact service string matches
      ...
      setShowPassword(false); // Reset pass visibility
  ...
  return (
    <div style={bgStyle}>
      ...
      {/* SERVICE DETAIL MODAL (WITH CREDENTIALS) */}
      {selectedService && (
          <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
              <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="p-6 space-y-6">
                      ...
                      {/* AREA DE CREDENCIAIS (LÓGICA CORRIGIDA) */}
                      {selectedService.isBlocked ? (
                          /* 1. BLOQUEADO POR DÍVIDA OU DATA */
                          ...
                          /* 2. ATIVO MAS SEM CREDENCIAL CONFIGURADA (PENDENTE DO ADMIN) */
                          ...
                          /* 3. ATIVO E COM CREDENCIAL (SUCESSO) */
      ...
      {/* GLOBAL DEBT WARNING BANNER - FONTE AUMENTADA */}
      ...
      <div className="px-4 space-y-6 pt-4">
             ...
             <div className="flex flex-col gap-3">
                 {user.services.length > 0 ? user.services.map((rawSvc, i) => {
                     ...
                     if (isBlocked) { 
                         ...
                         // CONTA ATIVA E SEGURA: BOTÃO ADIANTAR RENOVAÇÃO
                         if (daysLeft <= 5) {
        ...
        {/* --- ASSISTENTE IA (NOVA LOCALIZAÇÃO E ESTILO PROFISSIONAL) --- */}
        <button 
            onClick={onOpenSupport}
            className="w-full bg-white border border-indigo-100 p-4 rounded-2xl shadow-sm shadow-indigo-100 flex items-center gap-4 group transition-all active:scale-95 relative overflow-hidden hover:shadow-md"
        >
            ...
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-3 rounded-xl text-white shadow-md group-hover:scale-110 transition-transform duration-300">
                <Cast className="w-6 h-6" />
            </div>
            <div className="flex-1 text-left relative z-10">
                <span className="font-extrabold text-gray-900 text-base block mb-0.5">Conectar na TV e Celular</span>
                <span className="text-xs text-gray-500 font-medium block">Guia passo a passo para liberar o acesso</span>
            </div>
            <div className="bg-gray-50 p-2 rounded-full group-hover:bg-indigo-50 transition-colors relative z-10">
                <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-indigo-500" />
            </div>
        </button>

        {/* SECTION: CREDENCIAIS EM TEMPO REAL (NOVO) */}
            ...
            <div className="mt-2">
                ...
                <div className="flex flex-col gap-3">
                    {assignedCredentials.map((item, idx) => {
                        ...
                        // Logic for "Atualiza em..."
                        const isViki = item.service.toLowerCase().includes('viki');
                        const cycleDays = isViki ? 14 : 30; // Viki 14 days, others 30
        ...
        {/* CATÁLOGO DE NOVOS SERVIÇOS (DESTACADO E ANIMADO) */}
        {missingServices.length > 0 && (
            <div className="mt-8 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-6 rounded-[2rem] border border-purple-100 shadow-lg relative overflow-hidden">
                    ...
                    {missingServices.map(svc => (
                        <div key={svc.id} className="bg-white p-5 rounded-2xl border border-purple-50 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col justify-between group h-full">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-3">{svc.benefits[0]}</p>
                            </div>
                ...
                {/* Decorative Elements */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-purple-200 to-pink-200 rounded-full blur-3xl opacity-20 -mr-20 -mt-20 pointer-events-none"></div>
                <div className="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-tr from-blue-200 to-indigo-200 rounded-full blur-3xl opacity-20 -ml-10 -mb-10 pointer-events-none"></div>
            </div>
        )}

        {/* BOTÕES DE AÇÃO RÁPIDA REMOVIDOS (JOGOS JÁ ESTÃO NA BARRA INFERIOR) */}
